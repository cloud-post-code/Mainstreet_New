#!/usr/bin/env python3
"""
Scrape Kodomo Boston (kodomoboston.com) — defaults to the "all" collection.

Produces the same CSV format used by the other Shopify shop scrapes so the
existing upload pipeline can ingest it.

Usage:
  python3 scrape_kodomoboston_shop_all.py
  python3 scrape_kodomoboston_shop_all.py --collection best-sellers
  python3 scrape_kodomoboston_shop_all.py --resume
"""
...
DOMAIN = "https://www.kodomoboston.com"
SHOP_ALL_SLUG = "all"
COLLECTION_SLUG = SHOP_ALL_SLUG
COLLECTION_URL = f"{DOMAIN}/collections/{SHOP_ALL_SLUG}"
OUTPUT_CSV = Path(__file__).resolve().parent / "kodomoboston_shop_all.csv"
...
USER_AGENT = "Mozilla/5.0 (compatible; KodomoBostonCatalog/1.0; +https://www.kodomoboston.com)"
...
DEFAULT_SHOP_NAME = "Kodomo Boston"
...
def scrape_product(handle: str) -> list[dict]:
    ...
    for variant_index, variant in enumerate(variants, start=1):
        ...
        if vendor:
            description_json["vendor"] = vendor
        if product_type:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Kodomo Boston collection")
...
def configure(
    ...
    if output:
        ...
        base = "kodomoboston_shop_all" if COLLECTION_SLUG == SHOP_ALL_SLUG else f"kodomoboston_{COLLECTION_SLUG.replace('/', '_')}"
