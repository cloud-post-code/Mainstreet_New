#!/usr/bin/env python3
"""
Scrape Wyllo (shopwyllo.com) — defaults to the "art-prints" collection.

Produces the same CSV format used by the other Shopify shop scrapes so the
existing upload pipeline can ingest it.

Usage:
  python3 scrape_shopwyllo_art_prints.py
  python3 scrape_shopwyllo_art_prints.py --collection all
  python3 scrape_shopwyllo_art_prints.py --resume
"""
...
DOMAIN = "https://www.shopwyllo.com"
DEFAULT_COLLECTION_SLUG = "art-prints"
COLLECTION_SLUG = DEFAULT_COLLECTION_SLUG
COLLECTION_URL = f"{DOMAIN}/collections/{COLLECTION_SLUG}"
OUTPUT_CSV = Path(__file__).resolve().parent / "shopwyllo_art_prints.csv"
...
USER_AGENT = "Mozilla/5.0 (compatible; WylloCatalog/1.0; +https://www.shopwyllo.com)"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Wyllo collection")
...
def configure(
    ...
    if output:
        ...
        base = f"shopwyllo_{COLLECTION_SLUG.replace('/', '_').replace('-', '_')}"
