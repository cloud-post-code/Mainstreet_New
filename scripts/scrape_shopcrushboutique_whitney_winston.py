#!/usr/bin/env python3
"""
Scrape Crush Boutique (shopcrushboutique.com) — defaults to the
"whitney-winston" collection.

Produces the same CSV format used by the other Shopify scrapes so the
existing upload pipeline can ingest it:
...
Usage:
  python3 scrape_shopcrushboutique_whitney_winston.py
  python3 scrape_shopcrushboutique_whitney_winston.py --collection all
  python3 scrape_shopcrushboutique_whitney_winston.py --resume
"""
...
DOMAIN = "https://shopcrushboutique.com"
DEFAULT_COLLECTION_SLUG = "whitney-winston"
COLLECTION_SLUG = DEFAULT_COLLECTION_SLUG
COLLECTION_URL = f"{DOMAIN}/collections/{COLLECTION_SLUG}"
OUTPUT_CSV = Path(__file__).resolve().parent / "shopcrushboutique_whitney_winston.csv"
...
USER_AGENT = "Mozilla/5.0 (compatible; CrushBoutiqueCatalog/1.0; +https://shopcrushboutique.com)"
PRODUCT_URL_BASE = f"{DOMAIN}/products"
DEFAULT_VENDOR = "Crush Boutique"
...
def scrape_product(handle: str) -> list[dict]:
    ...
    vendor = product.get("vendor") or DEFAULT_VENDOR
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Crush Boutique collection")
    parser.add_argument("--collection", default=DEFAULT_COLLECTION_SLUG)
...
def configure(
    ...
    if output:
        ...
        base = f"shopcrushboutique_{COLLECTION_SLUG.replace('/', '_')}"
