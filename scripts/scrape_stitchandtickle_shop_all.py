#!/usr/bin/env python3
"""
Scrape Stitch and Tickle (stitchandtickle.com) — defaults to the "all"
collection.
...
Usage:
  python3 scrape_stitchandtickle_shop_all.py
  python3 scrape_stitchandtickle_shop_all.py --collection best-sellers
  python3 scrape_stitchandtickle_shop_all.py --resume
"""
...
DOMAIN = "https://www.stitchandtickle.com"
SHOP_ALL_SLUG = "all"
COLLECTION_SLUG = SHOP_ALL_SLUG
COLLECTION_URL = f"{DOMAIN}/collections/{SHOP_ALL_SLUG}"
OUTPUT_CSV = Path(__file__).resolve().parent / "stitchandtickle_shop_all.csv"
...
USER_AGENT = "Mozilla/5.0 (compatible; StitchAndTickleCatalog/1.0; +https://www.stitchandtickle.com)"
...
def scrape_product(handle: str) -> list[dict]:
    ...
    vendor = product.get("vendor") or "Stitch and Tickle"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Stitch and Tickle collection")
...
def configure(
    ...
    if output:
        ...
        base = "stitchandtickle_shop_all" if COLLECTION_SLUG == SHOP_ALL_SLUG else f"stitchandtickle_{COLLECTION_SLUG.replace('/', '_')}"
