#!/usr/bin/env python3
"""
Scrape Topdrawer (topdrawershop.com) — defaults to the "all" collection.

Produces the same CSV format used by Boston General Store scrape so the
...
shop_name comes from the Shopify "vendor" field (Topdrawer body_html has no
labeled metadata block like Boston General Store).

Usage:
  python3 scrape_topdrawer_shop_all.py
  python3 scrape_topdrawer_shop_all.py --collection best-sellers
  python3 scrape_topdrawer_shop_all.py --resume
"""
...
DOMAIN = "https://topdrawershop.com"
SHOP_ALL_SLUG = "all"
COLLECTION_SLUG = SHOP_ALL_SLUG
COLLECTION_URL = f"{DOMAIN}/collections/{SHOP_ALL_SLUG}"
OUTPUT_CSV = Path(__file__).resolve().parent / "topdrawer_shop_all.csv"
...
USER_AGENT = "Mozilla/5.0 (compatible; TopdrawerCatalog/1.0; +https://topdrawershop.com)"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scrape Topdrawer collection")
...
def configure(
    ...
    if output:
        ...
        base = "topdrawer_shop_all" if COLLECTION_SLUG == SHOP_ALL_SLUG else f"topdrawer_{COLLECTION_SLUG.replace('/', '_')}"
